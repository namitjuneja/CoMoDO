...
def compute_distance(x1, y1, image_set_directory, query_image_filename):
    with open('./testing.txt', 'w') as f:
        f.write(query_image_filename)
    image_vectors = []
    ...
    source_directory_files = [i for i in source_directory.iterdir()]
    source_directory_files_sorted = sorted(source_directory_files, key=lambda x: x.name.split(".")[1])
    files_to_be_considered = [source_directory_files_sorted[-1]] # only compare the last file of the morphology
 
    for idx, image in enumerate(files_to_be_considered):
    ...
    import time; 
    a = int(time.time())
    with open(f'/panasas/scratch/grp-jzola/namitjun/PhaFiCContainer/server/zzz.txt', 'w') as f:
        f.write(str(min(distances)))
        f.write("probabaola")
...
def front_pad(vector, max_dimension):
    ...
    return np.pad(vector, (0,max_dimension-len(vector)), mode='constant')
